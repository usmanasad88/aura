...
Milk-tea-making variant of ``field_accuracy_eval`` (which targets the
tea_making schema of water + powdered milk). This one grades the *categorical*
and *boolean* fields of the milk_tea_making schema (fresh milk simmered with
tea, then strained). Each scored field contributes one binary "item" per
prediction frame; overall accuracy is the fraction of items correct.

Scored fields
-------------
Categorical (string equality):
    current_phase, current_action
Boolean (equality):
    milk_in_pot, water_added, pot_on_stove, stove_on, milk_boiling,
    tea_added, sugar_added, tea_simmered, stove_off, tea_strained
...
::
    ...
    # logs/Milk_Tea_Making/ against
    # tasks/milk_tea_making/ground_truth/iiith_cooking_03_3_aria_cut_overlay.intent_gt.json
    python scripts/eval/field_accuracy_eval_milk_tea.py

    # Score one specific predictions file
    python scripts/eval/field_accuracy_eval_milk_tea.py \\
        --gt   tasks/milk_tea_making/ground_truth/iiith_cooking_03_3_aria_cut_overlay.intent_gt.json \\
        --pred logs/Milk_Tea_Making/.../predictions.json
"""
...
DEFAULT_PRED_DIR = AURA_ROOT / "logs" / "Milk_Tea_Making"
DEFAULT_GT_PATH = (
    AURA_ROOT / "tasks" / "milk_tea_making" / "ground_truth"
    / "iiith_cooking_03_3_aria_cut_overlay.intent_gt.json"
)
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Per-field accuracy of intent-monitor predictions vs. ground truth (milk_tea_making)",
    )
