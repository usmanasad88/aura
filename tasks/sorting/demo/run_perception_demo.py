#!/usr/bin/env python3
"""Standalone demo: track regions & objects in a sorting video.

Runs the task-specific ``SortingPerceptionMonitor`` frame-by-frame on a
video file and displays masks + object-to-region assignments.
...
``logs/sorting/perception_demo/<timestamp>/``:
  - ``frame_NNNNN.jpg`` — annotated visualization per processed frame
  - ``results.jsonl`` — one JSON object per processed frame with
    object locations, region assignments, detections, and timing

Usage::

    uv run python tasks/sorting/demo/run_perception_demo.py \\
        --video demo_data/sorting_demo/sorting_demo.mp4 \\
        --frame-skip 30
...
<title>Sorting Perception Demo</title>
...
def _ensure_numpy_masks(result: dict) -> None:
    ...
    # Region objects (table, baskets).
    for region_id, robjs in result.get("regions", {}).items():
        for obj in robjs:
    ...
    # Movable objects.
    for prompt, objs in result.get("objects", {}).items():
        for obj in objs:
            if obj.mask is not None:
                m = obj.mask
                if hasattr(m, "cpu"):
...
def run_demo(
    ...
    config = SortingPerceptionConfig()
    monitor = SortingPerceptionMonitor(config=config)
    ...
    log_dir = _project_root / "logs" / "sorting" / "perception_demo" / run_ts
    ...
    try:
        while True:
            ...
            processed += 1
            locs = result["object_locations"]
            detected = result.get("detected_this_frame", set())
            det_counts = {k: len(v) for k, v in result["detections"].items()}
            logger.info(
                "Frame %5d | %6.1fs | %.2fs | detected=%d/%d | raw_dets=%s",
                frame_num, timestamp, dt, len(detected),
                len(locs), det_counts,
            )
            for oid in sorted(locs):
                tag = "" if oid in detected else " (last known)"
                logger.info("  %s: %s%s", oid, locs[oid], tag)
            ...
            region_counts = " | ".join(
                f"{k}: {len(v)}"
                for k, v in result.get("regions", {}).items()
            )
            info = (f"Frame {frame_num} | {timestamp:.1f}s | "
                    f"SAM3: {dt:.2f}s | {region_counts}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sorting perception demo — track regions & objects with SAM3",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--video", required=True,
        help="Path to sorting demo video",
    )
    parser.add_argument(
