#!/usr/bin/env python3
"""Standalone demo: locate tea-making items in a kettle-tea-making video.

Runs the task-specific ``KettlePerceptionMonitor`` frame-by-frame on a video
file and displays region overlays (blue basket = storage, table = working
area) plus per-item location labels.

Items located: tea (container with a black lid), paper cup, water bottle,
biscuits (red), and powdered milk (container with a light blue cap).  Each is
classified as ``storage_area`` (in the blue basket), ``working_area`` (on the
workspace table), or ``other``.

Visualization: serves a live-updating JPEG at ``http://localhost:<port>``
via a lightweight HTTP server (avoids OpenCV/matplotlib GUI crashes caused by
SAM3 CUDA + Qt5 conflict).

Outputs are saved to a timestamped run folder under
``logs/kettle_tea_making/perception_demo/<timestamp>/``:
  - ``images/frame_NNNNN.jpg`` — annotated visualization per processed frame
  - ``results.jsonl`` — one JSON object per processed frame with item
...
<title>Kettle Tea Perception Demo</title>
...
def _ensure_numpy_masks(result: dict) -> None:
    ...
    objs = list(result.get("items", {}).values())
    objs.extend(result.get("regions", {}).values())
    objs.extend(result.get("markers", {}).values())
    for obj in objs:
        if obj is not None and getattr(obj, "mask", None) is not None:
            m = obj.mask
            if hasattr(m, "cpu"):
                m = m.cpu().numpy()
...
def run_demo(
    ...
    log_dir = (_project_root / "logs" / "kettle_tea_making"
               / "perception_demo" / run_ts)
    ...
    try:
        while True:
            ...
            processed += 1
            locs = result["item_locations"]
            det_counts = {k: len(v) for k, v in result["detections"].items()}
            logger.info(
                "Frame %5d | %6.1fs | %.2fs | items=%s",
                frame_num, timestamp, dt, locs,
            )
            ...
            info = (f"Frame {frame_num} | {timestamp:.1f}s | "
                    f"SAM3: {dt:.2f}s | dets={sum(det_counts.values())}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Kettle tea-making perception demo — locate items with SAM3",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--video", required=True,
        help="Path to the kettle tea-making demo video",
    )
    parser.add_argument(
